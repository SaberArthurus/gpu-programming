...
__global__ void gamma_correction (float *d_imgIn, float *d_imgOut, float gamma, int size)
{
    size_t ind = threadIdx.x + threadIdx.y * blockDim.x + blockIdx.x * blockDim.x * blockDim.y ;
    if (ind < size)
    {
        d_imgOut[ind] = pow(d_imgIn[ind], gamma);
    }
}
...
int main(int argc, char **argv)
{
    ...
    // get the gamma parameter which will later be used for gamma-correction
    float gamma = 0;
    ...
    dim3 block = dim3(128, 2, 1); 
    dim3 grid = dim3((w * h * nc + block.x * block.y - 1) / (block.x * block.y), 1, 1);
   
    Timer timer; timer.start();

    for (int rep = 0; rep < repeats; rep++)
    {
        gamma_correction <<<grid, block>>> (d_imgIn, d_imgOut, gamma, imgSize);
    }
    timer.end();  float t = timer.get();  // elapsed time in seconds
    cout << "average time: " << t*1000/repeats << " ms" << endl;
