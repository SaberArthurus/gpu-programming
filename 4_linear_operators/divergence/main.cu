...
__global__ void divergence (float *d_imgIn, float *d_imgDiv, int w, int h, int nc, int size)
{
...
int main(int argc, char **argv)
{
    ...
    float *imgDiv = new float[(size_t)w * h * mDiv.channels()];
    ...
    cudaMalloc(&d_imgDiv, nbytes); CUDA_CHECK;
    ...
    {
        divergence <<<grid, block>>> (d_imgIn, d_imgDiv, w, h, nc, size);   
    }
    timer.end();  float t = timer.get();  // elapsed time in seconds
    cout << "average kernel time: " << t*1000/repeats << " ms" << endl;

    // copy result back to host memory
    cudaMemcpy(imgDiv, d_imgDiv, nbytes, cudaMemcpyDeviceToHost); CUDA_CHECK;

    // free the device memory
    cudaFree(d_imgIn); CUDA_CHECK;
    cudaFree(d_imgDiv); CUDA_CHECK;

    // // show input image
    // showImage("Input", mIn, 100, 100);  // show at position (x_from_left=100,y_from_above=100)

    // // show output image: first convert to interleaved opencv format from the layered raw array
    convert_layered_to_mat(mDiv, imgDiv);
    // showImage("Divergence", mDiv, 100+w+40, 100);
    ...
    cv::imwrite("divergence.png", mDiv * 255.f);
